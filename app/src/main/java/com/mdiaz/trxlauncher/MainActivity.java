...
public class MainActivity extends Activity {
    ...
    @Override public void onCreate(Bundle state){super.onCreate(state);getWindow().setStatusBarColor(Ui.BG);getWindow().setNavigationBarColor(Ui.BG);requestCorePermissions();buildShell(state);}
    ...
    public void showPage(int p){page=p;View oldSettings=stage.findViewWithTag("settings-screen");if(oldSettings!=null)stage.removeView(oldSettings);for(int i=0;i<pages.length;i++)pages[i].setVisibility(i==p?View.VISIBLE:View.GONE);
    ...
    private void requestCorePermissions(){ArrayList<String> needs=new ArrayList<>();if(checkSelfPermission(Manifest.permission.ACCESS_FINE_LOCATION)!=PackageManager.PERMISSION_GRANTED)needs.add(Manifest.permission.ACCESS_FINE_LOCATION);if(android.os.Build.VERSION.SDK_INT>=31&&checkSelfPermission(Manifest.permission.BLUETOOTH_CONNECT)!=PackageManager.PERMISSION_GRANTED)needs.add(Manifest.permission.BLUETOOTH_CONNECT);if(!needs.isEmpty())requestPermissions(needs.toArray(new String[0]),41);}
