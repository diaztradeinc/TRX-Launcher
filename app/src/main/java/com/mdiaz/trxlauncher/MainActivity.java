...
public class MainActivity extends Activity {
    ...
    private void requestBluetoothIfNeeded(){if(!ObdService.hasBluetoothPermission(this)){requestPermissions(new String[]{Manifest.permission.BLUETOOTH_CONNECT},42);return;}startObdIfPermitted();}
    private void startObdIfPermitted(){if(page!=3||!ObdService.hasBluetoothPermission(this))return;try{startForegroundService(new Intent(this,ObdService.class));}catch(IllegalStateException|SecurityException e){android.util.Log.w("TRX-OBD","OBD service start rejected",e);Toast.makeText(this,"OBD connection unavailable; launcher remains usable",Toast.LENGTH_LONG).show();}}
    @Override public void onRequestPermissionsResult(int requestCode,String[] permissions,int[] results){super.onRequestPermissionsResult(requestCode,permissions,results);if(requestCode==41&&results.length>0&&results[0]==PackageManager.PERMISSION_GRANTED)navigation.ensureNavigator();if(requestCode==42){if(results.length>0&&results[0]==PackageManager.PERMISSION_GRANTED)startObdIfPermitted();else Toast.makeText(this,"Bluetooth access declined. Other pages remain available.",Toast.LENGTH_LONG).show();}}
